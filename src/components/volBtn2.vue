      ...
      <div @mousewheel.passive="wheelFn" :class="{'slider-box':true, 'slider-box-active':DRAG_START, 'drag-region':true}">
        <div class="slider-box-box" @mousedown="drag_start($event,1)">
          <div ref="vol-progress" class="vol-progress">
            <div :style="{width: audio_volume*100+'%'}" :class="{'vol-pro-er':true, 'vol-pro-er-active':DRAG_START}"></div>
            <div :style="{left: (audio_volume)*100+'%'}" :class="{'vol-pro-dot':true, 'vol-pro-dot-active':DRAG_START}"
...
export default {
  ...
  methods: {
    ...
    drag_on(event){
      ...
      let curOffset = (event.clientX - this.START_CLIENTX) + this.START_offsetX
      if(curOffset>this.$refs['vol-progress'].offsetWidth){
        curOffset = this.$refs['vol-progress'].offsetWidth
      }else if(curOffset<0){
        curOffset = 0
      }
      this.Normalize((curOffset / this.$refs['vol-progress'].offsetWidth))
    },
    drag_end(){
    ...
    drag_start(event, opt){
      if(opt==1){
        this.START_offsetX = event.offsetX
        this.Normalize((event.offsetX / this.$refs['vol-progress'].offsetWidth))
      }else{
        this.START_offsetX = event.target.offsetLeft
      }
      setTimeout(() => {
        this.DRAG_START = true
      }, 0);
      this.START_CLIENTX = event.clientX
